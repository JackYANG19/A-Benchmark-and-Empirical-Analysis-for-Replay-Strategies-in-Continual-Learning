...
parser.add_argument('-d', '--dataset', type=str, default='split_mnist',
                    choices=['split_mnist', 'permuted_mnist', 'split_cifar10',
                             'split_cifar100', 'miniimagenet', 'openloris'])

#---------- train order -----------#
parser.add_argument('--hardfirst', dest='hardfirst',
                    action='store_true', help='OpenLORIS: train with hard samples first')
parser.add_argument('--no-hardfirst', dest='hardfirst',
                    action='store_false', help='OpenLORIS: train with easy samples first')
#----------------------------------#

parser.add_argument('--n_tasks', type=int, default=-1,
                    help='total number of tasks. -1 does default amount for the dataset')
# Default No reproduction
...
result_path = os.path.join('GR_res', args.result_dir)
if not os.path.exists(result_path):
    os.mkdir(result_path)
sample_path = os.path.join(*['GR_res', args.result_dir, 'samples/'])
if not os.path.exists(sample_path):
    os.mkdir(sample_path)
recon_path = os.path.join(*['GR_res', args.result_dir, 'reconstructions/'])
if not os.path.exists(recon_path):
    os.mkdir(recon_path)
mir_path = os.path.join(*['GR_res', args.result_dir, 'mir/'])
...
if args.log != 'off':
    ...
    if args.name is None:
        wandb.init(project=args.result_dir)
    else:
        wandb.init(project=args.result_dir, name=args.name)
    ...
    for task, tr_loader in enumerate(train_loader):
        ...
        for epoch in range(args.n_epochs):
            ...
            for i, (data, target) in enumerate(tr_loader):
                ...
                args.beta = min(
                    [(sample_amt) / max([args.warmup, 1.]), args.max_beta])
                    ...
                    if task > 0 and args.gen_method != 'no_rehearsal':
                        ...
                        if it == 0 or not args.reuse_samples:
                            ...
                            if args.gen_method == 'rand_gen':
                                mem_x = prev_gen.generate(
                                    args.batch_size * args.n_mem).detach()

                            elif args.gen_method == 'mir_gen':

                                mem_x, mir_worked = \
                                    mir.retrieve_gen_for_gen(
                                        args, data, gen, prev_gen, prev_cls)
                ...
                for it in range(args.cls_iters):
                    ...
                    if task > 0 and args.method != 'no_rehearsal':
                        ...
                        if it == 0 or not args.reuse_samples:
                            ...
                            if args.method == 'rand_gen':
                                mem_x = prev_gen.generate(
                                    args.batch_size * args.n_mem).detach()
                                mem_y = torch.softmax(
                                    prev_cls(mem_x), dim=1).detach()

                            elif args.method == 'mir_gen':

                                mem_x, mem_y, mir_worked = \
                                    mir.retrieve_gen_for_cls(
                                        args, data, cls, prev_cls, prev_gen)
                        ...
                        mem_cls_loss = naive_cross_entropy_loss(
                            mem_logits, mem_y)
        ...
        with torch.no_grad():
            ...
            if gen is not None:
                ...
                if args.dataset is not 'permuted_mnist':
                    ...
                    recon_path_ = os.path.join(
                        recon_path, 'task{}.png'.format(task))
                    ...
                    if wandb is not None:
                        ...
                        wandb.log({"Reconstructions Run {}".format(
                            run): logged_recons}, step=task)

                    # save some pretty images:
                    gen_images = gen.generate(25).to('cpu')
                    sample_path_ = os.path.join(
                        sample_path, 'task{}.png'.format(task))
                    save_image(gen_images, sample_path_, nrow=5)
                    if wandb is not None:
                        logged_gen = wandb.Image(gen_images[:16],
                                                 caption="generations task {}".format(task))
                        wandb.log({"Generations Run {}".format(
                            run): logged_gen}, step=task)
            ...
            for mode, loader_ in eval_loaders:
                ...
                for task_t, te_loader in enumerate(loader_):
                    if task_t > task:
                        break
                    LOG_temp = get_temp_logger(
                        None, ['gen_loss', 'cls_loss', 'acc'])
                    ...
                    for i, (data, target) in enumerate(te_loader):
                        ...
                        LOG_temp['acc'] += [pred.eq(target.view_as(pred)
                                                    ).sum().item() / pred.size(0)]
...
for mode in ['valid', 'test']:
    ...
    # MIR worked
    try:
        final_mir_worked_frac = [
            LOG[x][mode]['final_mir_worked_frac'] for x in range(args.n_runs)]
        final_mir_worked_avg = np.mean(final_mir_worked_frac)
    except:
        pass

    print('\nFinal {} Accuracy: {:.3f} +/- {:.3f}'.format(mode,
          final_acc_avg, final_acc_se))
    print('\nFinal {} Forget: {:.3f} +/- {:.3f}'.format(mode,
          final_forget_avg, final_forget_se))
    print('\nFinal {} ELBO: {:.3f} +/- {:.3f}'.format(mode,
          final_elbo_avg, final_elbo_se))
    print('\nAverage time is {:.3f} seconds'.format(
        processtime / args.n_runs))
