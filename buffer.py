...
class Buffer(nn.Module):
    def __init__(self, args, input_size=None):
        ...
        if args.gen:
            if 'mnist' in args.dataset:
                ...
                economy = img_size // (input_size[0] ** 2)
            #---------- openloris ----------#
            elif 'openloris' in args.dataset:
                img_size = 84 * 84 * 3
                economy = img_size // (input_size[0] ** 2)
        else:
            economy = 1
        ...
        self.to_one_hot = lambda x: x.new(x.size(0), args.n_classes).fill_(
            0).scatter_(1, x.unsqueeze(1), 1)
    ...
    def display(self, gen=None, epoch=-1):
        ...
        save_image((x.reshape(shp) * 0.5 + 0.5), 'samples/buffer_%d.png' %
                   epoch, nrow=int(self.current_index ** 0.5))
    ...
    def add_reservoir(self, x, y, logits, t):
        ...
        if place_left:
            ...
            self.bx[self.current_index: self.current_index +
                    offset].data.copy_(x[:offset])
            self.by[self.current_index: self.current_index +
                    offset].data.copy_(y[:offset])
            self.bt[self.current_index: self.current_index + offset].fill_(t)

            if save_logits:
                self.logits[self.current_index: self.current_index +
                            offset].data.copy_(logits[:offset])
        ...
        indices = torch.FloatTensor(x.size(0)).to(
            x.device).uniform_(0, self.n_seen_so_far).long()
    ...
    def sample(self, amt, exclude_task=None, ret_ind=False):
        if exclude_task is not None:
            ...
            bx, by, bt = self.bx[:self.current_index], self.by[:
                                                               self.current_index], self.bt[:self.current_index]
        ...
        if bx.size(0) < amt:
            ...
            indices = torch.from_numpy(
                np.random.choice(bx.size(0), amt, replace=False))
