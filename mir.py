...
def dist_kl(y, t_s): return F.kl_div(F.log_softmax(y, dim=-1),
                                     F.softmax(t_s, dim=-1), reduction='mean') * y.size(0)
# this returns -entropy


def entropy_fn(x): return torch.sum(
    F.softmax(x, dim=-1) * F.log_softmax(x, dim=-1), dim=-1)


def cross_entropy(y, t_s): return -torch.sum(F.log_softmax(y,
                                                           dim=-1) * F.softmax(t_s, dim=-1), dim=-1).mean()
            ...
            if args.cls_div_coeff > 0.:
                for found_z_i in range(i):
                    DIV += F.mse_loss(
                        z_new,
                        z_new_max[found_z_i *
                                  z_new.size(0):found_z_i * z_new.size(0) + z_new.size(0)]
                    ) / (i)
...
def retrieve_gen_for_gen(args, x, gen, prev_gen, prev_cls):
    ...
    virtual_gen = get_future_step_parameters(
        gen, grad_vector, gen.grad_dims, args.lr)
    ...
    for i in range(args.n_mem):
        ...
        for j in range(args.mir_iters):
            ...
            _, virtual_rec, virtual_kl, _ = calculate_loss(virtual_x_mean, x_new, virtual_z_mu,
                                                           virtual_z_var, virtual_z0, virtual_zk, virtual_ldj, args,
                                                           beta=1)
...
def retrieve_hybrid(args, model, gen, prev_model, prev_gen, input_x, input_y, buffer, task):
    ...
    for budget in range(args.max_loss_budget):
        ...
        for step in range(args.max_loss_grad_steps):
            ...
            if args.both_entropy:
                entropy += entropy_fn(y_new)

            # 3) diversity
            EUC = 0
            for found_h_i in range(budget if args.euc_coef > 0 else 0):
                EUC += F.mse_loss(hid_new, hid_new_max[found_h_i * hid_new.size(
                    0):(found_h_i + 1) * hid_new.size(0)])
    ...
    with torch.no_grad():
        ...
        # TODO( 5 is arbitrary )
        bx, by, bt = buffer.sample(
            5 * args.buffer_batch_size, exclude_task=task)
        ...
        out_x = out_x.unsqueeze(0).expand(
            n_repeats, -1, -1).reshape(n_repeats * out_x.size(0), out_x.size(1))
        out_y = out_y.unsqueeze(0).expand(
            n_repeats, -1).reshape(n_repeats * out_y.size(0))
        out_t = out_t.unsqueeze(0).expand(
            n_repeats, -1).reshape(n_repeats * out_t.size(0))
...
def retrieve_replay_update(args, model, opt, input_x, input_y, buffer, task, loader=None, rehearse=True):
    ...
    if args.multiple_heads:
        logits = logits.masked_fill(
            loader.dataset.mask == 0, -1e9)  # Multi-task or not
    ...
    if args.method == 'mir_replay':
        bx, by, bt, subsample = buffer.sample(
            args.subsample, exclude_task=task, ret_ind=True)
        grad_dims = []
        for param in model.parameters():
            grad_dims.append(param.data.numel())  # same dim
        # make gradients in one vector
        grad_vector = get_grad_vector(args, model.parameters, grad_dims)
        model_temp = get_future_step_parameters(
            model, grad_vector, grad_dims, lr=args.lr)  # \theta-\delta \theta

        with torch.no_grad():
            logits_track_pre = model(bx)  # bx is bc in the paper
            ...
            all_logits = scores
            big_ind = all_logits.sort(descending=True)[
                1][:args.buffer_batch_size]
    ...
    if args.method == 'entropy':
        ...
        with torch.no_grad():
            logits_track = model(bx)
            # Softmax
            logits_track_pre = F.softmax(logits_track.float(), dim=1)
            log_probs = torch.log(logits_track_pre)
            scores = (logits_track_pre * log_probs).sum(1)

            all_logits = scores
            big_ind = all_logits.sort(descending=args.simple)[
                1][:args.buffer_batch_size]  # buffer_batch_size is # bmc
            idx = subsample[big_ind]
    ...
    if args.method == 'bayesian':
        ...
        with torch.no_grad():
            n_drop = 10
            probs = torch.zeros(
                [n_drop, bx.shape[0], args.n_classes]).to(args.device)
            for i in range(n_drop):
                logits_track = model(bx)
                logits_track_pre = F.softmax(logits_track.float(), dim=1)
                probs[i] += logits_track_pre

            pb = probs.mean(0)
            entropy1 = (-pb*torch.log(pb)).sum(1)
            entropy2 = (-probs*torch.log(probs)).sum(2).mean(0)
            scores = - (entropy2 - entropy1)
    ...
    if args.method == 'confidence':
        ...
        with torch.no_grad():
            ...
            scores = logits_track_pre.max(1)[0]
    ...
    if args.method == 'margin':
        ...
        with torch.no_grad():
            logits_track_pre = model(bx)
            logits_track_pred = logits_track_pre.sort(descending=True)[0]
            scores = logits_track_pred[:, 0] - logits_track_pred[:, -1]
    ...
    if args.method == 'kmeans':
        ...
        with torch.no_grad():
            embedding = model.return_embedding(bx).cpu()
            embedding = embedding.numpy()
            cluster_learner = KMeans(n_clusters=args.buffer_batch_size)
            ...
            dis = dis.sum(axis=1)
            big_ind = np.array([np.arange(embedding.shape[0])[
                cluster_idxs == i][dis[cluster_idxs == i].argmin()] for i in range(args.buffer_batch_size)])
    ...
    if args.method == 'coreset':
        ...
        with torch.no_grad():
            ...
            embedding = embedding.numpy()
            # random.seed(39)
            start_ind = random.randint(0, embedding.shape[0]-1)
            lb_flag = np.zeros((embedding.shape[0],), dtype=bool)
            lb_flag[start_ind] = True
            ...
            sq = np.array(dist_mat.diagonal()).reshape(embedding.shape[0], 1)
            dist_mat *= -2
            dist_mat += sq
            ...
            for i in range(args.buffer_batch_size):
                # if i % 10 == 0:
                #print('greedy solution {}/{}'.format(i, args.buffer_batch_size))
                mat_min = mat.min(axis=1)
                q_idx_ = mat_min.argmax()
                q_idx = np.arange(embedding.shape[0])[~lb_flag][q_idx_]
                lb_flag[q_idx] = True
                mat = np.delete(mat, q_idx_, 0)
                mat = np.append(
                    mat, dist_mat[~lb_flag, q_idx][:, None], axis=1)
            big_ind = np.arange(embedding.shape[0])[lb_flag]
            idx = subsample[big_ind]
        mem_x, mem_y, logits_y, b_task_ids = bx[big_ind], by[big_ind], buffer.logits[idx], bt[big_ind]
