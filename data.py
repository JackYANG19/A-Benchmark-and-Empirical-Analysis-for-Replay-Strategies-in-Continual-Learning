...
class XYDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if type(x) != torch.Tensor:
            # mini_imagenet / openloris
            # we assume it's a path --> load from file
            '''
            remove .convert(RGB)
            '''
            x = self.transform(Image.open(x))
    ...
    def __init__(self, datasets_per_task, args, train=True):
        ...
        self.loaders = [
            torch.utils.data.DataLoader(
                x, batch_size=bs, shuffle=True, drop_last=train, num_workers=0)
...
def get_permuted_mnist(args):
    ...
    for task in range(args.n_tasks):
        perm = torch.arange(
            train_x.size(-1)) if task == 0 else torch.randperm(train_x.size(-1))
    ...
    train_ds = map(lambda x, y: XYDataset(
        x[0], x[1], **{'inv_perm': y, 'source': 'mnist'}), train_ds, inv_perms)
    val_ds = map(lambda x, y: XYDataset(
        x[0], x[1], **{'inv_perm': y, 'source': 'mnist'}), val_ds, inv_perms)
    test_ds = map(lambda x, y: XYDataset(
        x[0], x[1], **{'inv_perm': y, 'source': 'mnist'}), test_ds, inv_perms)
...
def get_split_mnist(args):
    ...
    # cast in 3D:
    train_x = train_x.view(train_x.size(
        0), 1, train_x.size(1), train_x.size(2))
    ...
    train_ds = map(lambda x: XYDataset(
        x[0], x[1], **{'source': 'mnist'}), train_ds)
    val_ds = map(lambda x: XYDataset(
        x[0], x[1], **{'source': 'mnist'}), val_ds)
    test_ds = map(lambda x: XYDataset(
        x[0], x[1], **{'source': 'mnist'}), test_ds)
...
def get_split_cifar10(args):
    ...
    train_ds = map(lambda x: XYDataset(
        x[0], x[1], **{'source': 'cifar10'}), train_ds)
    val_ds = map(lambda x: XYDataset(
        x[0], x[1], **{'source': 'cifar10'}), val_ds)
    test_ds = map(lambda x: XYDataset(
        x[0], x[1], **{'source': 'cifar10'}), test_ds)

    return train_ds, val_ds, test_ds


def get_miniimagenet(args):
    ROOT_PATH = '/home/ra//Desktop/miniimagenet/materials/images'
    ROOT_PATH_CSV = '/home/ra//Desktop/miniimagenet/materials'
    ...
    def cat(x, y): return np.concatenate((x, y), axis=0)
    ...
    for i in range(args.n_classes):
        ...
        if current_train is None:
            ...
            current_train = cat(current_train[0], data_train), cat(
                current_train[1], label_train)
            current_test = cat(current_test[0], data_test), cat(
                current_test[1], label_test)
    ...
    train_ds, val_ds = make_valid_from_train(train_ds)
    train_ds = map(lambda x, y: XYDataset(x[0], x[1], **{'source': 'cifar100', 'mask': y, 'task_ids': task_ids,
                                                         'transform': transform}), train_ds, masks)
    val_ds = map(lambda x, y: XYDataset(x[0], x[1], **{'source': 'cifar100', 'mask': y, 'task_ids': task_ids,
                                                       'transform': transform}), val_ds, masks)
    test_ds = map(lambda x, y: XYDataset(x[0], x[1], **{'source': 'cifar100', 'mask': y, 'task_ids': task_ids,
...
def get_openloris(args):
    ...
    args.n_classes = 121
    args.multiple_heads = False
    args.n_classes_per_task = 121
    args.input_size = (3, 84, 84)
    #-------------------------#

    # TODO: change root
    ROOT = '/home/ra/Desktop/openloris/'

    if args.hardfirst:
        segments = [9, 8, 7, 6, 5, 4, 3, 2, 1]  # hard first
    else:
        segments = [1, 2, 3, 4, 5, 6, 7, 8, 9]  # easy first

    def get_data(setname):
        data = []
        for factor in ['clutter', 'illumination', 'occlusion', 'pixel']:
            task_data = []
            task_target = []
            for segment in list('segment'+str(x) for x in segments):  # easy
                # Segment: ROOT + SETNAME + FACTOR + SEGMENT1/2/...
                SEG = os.path.join(ROOT, setname, factor, str(segment))

                classNames = list(f for f in os.listdir(SEG)
                                  if not f.startswith('.'))
                classNames.sort()
                for i, className in enumerate(classNames):
                    OBJ = os.path.join(SEG, className)
                    frames = list(f for f in os.listdir(OBJ)
                                  if not f.startswith('.'))
                    for frame in frames:
                        DATA = os.path.join(OBJ, frame)
                        task_data.append(DATA)
                        task_target.append(i)
            data += [(task_data, task_target)]
        return data
    ...
    train_ds = map(lambda x: XYDataset(
        x[0], x[1], **{'source': 'openloris', 'transform': transform}), train)
    val_ds = map(lambda x: XYDataset(
        x[0], x[1], **{'source': 'openloris', 'transform': transform}), val)
    test_ds = map(lambda x: XYDataset(
        x[0], x[1], **{'source': 'openloris', 'transform': transform}), test)
