...
class CVAE(nn.Module):
    def __init__(self, d, args, **kwargs):
        ...
        self.decoder = nn.Sequential(
            ...
            nn.ConvTranspose2d(d, d, kernel_size=4, stride=2,
            ...
            nn.BatchNorm2d(d),
            nn.ReLU(inplace=True),
            nn.ConvTranspose2d(d, 3, kernel_size=4, stride=2,
                               padding=1, bias=False),
        )
        self.f = 4  # 8
        self.d = d
    ...
    def sample(self, size):
        sample = Variable(torch.randn(
            size, self.d * self.f ** 2), requires_grad=False)
        if self.cuda():
            sample = sample.cuda()
...
class CategoricalConditionalBatchNorm(torch.nn.Module):
    ...
    def __init__(self, num_features, num_cats, eps=2e-5, momentum=0.1, affine=True,
        ...
        if self.affine:
            self.weight = torch.nn.Parameter(
                torch.Tensor(num_cats, num_features))
    ...
    def forward(self, input, cats):
        ...
        if self.affine:
            shape = [input.size(0), self.num_features] + \
                (input.dim() - 2) * [1]
...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes, nf, input_size):
        ...
        #---------- embedding: number of embedding = 50 ----------#
        self.adapt = nn.AdaptiveAvgPool2d((1, 1))
        #self.fc1 = nn.Linear(nf * 4, 50)
        #---------------------------------------------------------#
        ...
        # hardcoded for now
        last_hid = nf * 8 * \
            block.expansion if input_size[1] in [8, 16, 21, 32, 42] else 640

        #---------- dropout ----------#
        self.fc2 = nn.Linear(last_hid, last_hid)
    ...
    def return_embedding(self, x):
        ...
        out = self.layer1(out)
        out = self.layer2(out)
        out = self.layer3(out)  # (N, nf*4(80), H, W)
        out = self.adapt(out)  # (N, nf*4(80), 1, 1)
        out = torch.flatten(out, 1)  # (N, nf*4(80))
        #embedding = self.fc1(out)  # (N, 50)
        return out
    #-------------------------------------------------------------------------#
    ...
    def return_hidden(self, x):
        ...
        out = out.view(out.size(0), -1)  # Linear
        #---------- dropout ----------#
        out = self.fc2(out)
...
class MLP(nn.Module):
    def __init__(self, args, num_classes=10, nf=400):
        ...
        #---------- embedding (50) ----------#
        self.embedding = nn.Linear(nf, 50)
        #---------- drop out ----------#
        self.dropout = nn.Dropout(p=0.2)
