...
parser.add_argument('-m', '--method', type=str, default='no_rehearsal', choices=['no_rehearsal',
                                                                                 'rand_replay', 'mir_replay', 'entropy',
                                                                                 'bayesian', 'confidence', 'margin',
                                                                                 'kmeans', 'coreset'])
parser.add_argument('--compare_to_old_logits',
                    action='store_true', help='uses old logits')
parser.add_argument('--reuse_samples', type=int, default=0)
parser.add_argument('--lr', type=float, default=0.05)

#------difficulty----#
parser.add_argument('--simple', dest='simple',
                    action='store_true')  # replay easy samples
parser.add_argument('--no-simple', dest='simple',
                    action='store_false')  # replay hard samples
...
if args.suffix is not '':
    folder = "./ER_res/"+str(args.dataset)+"/"+str(args.mem_size)
    dir = os.path.exists(folder)
    if not dir:
        os.makedirs(folder)
    name_log = args.suffix + '.log'
    name_log_txt = folder + '/' + name_log
...
#print(np.array(list(train_loader)).size)
